...
result = cursor.find({'Crash.cnin': '47'}, {'Crashes.cnin':1})
...
db.newFormat.find({'Crashes.cnin': '47'},{'Crashes.cnin.$': 1, name: 1})

db.newFormat.find({}, {'Operator':'Military - U.S. Army'})
...
db.newFormat.aggregate( [{ $unwind: "$Crashes" },{$group: {_id: {title: '$type'},SumOfDeaths: { $avg: '$Crashes.in_Air' }}},{$project: {_id: 0,title: '$_id.type',SumOfDeaths: 1}}])
